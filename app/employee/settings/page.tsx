...
export default function EmployeeSettingsPage() {
  ...
  const employeeOptions = useMemo(() => {
    ...
    return employees
      .filter((employee) =>
        `${employee.empname ?? ''} ${employee.email ?? ''} ${employee.empid}`
          .toLowerCase()
          .includes(normalized)
      )
      .slice(0, 8);
  ...
  const filteredAssignments = useMemo(() => {
    ...
    return assignments.filter((assignment) =>
      `${assignment.empname ?? ''} ${assignment.email ?? ''} ${assignment.actionname ?? ''} ${assignment.membertypename ?? ''}`
        .toLowerCase()
        .includes(normalized)
    );
          ...
          <Card className="h-fit rounded-lg border-slate-200 shadow-sm xl:sticky xl:top-6">
            <CardHeader className="border-b border-slate-100">
              <CardTitle className="flex items-center gap-2 text-lg">
                ...
                {draft.id ? 'Тохиргоо засах' : 'Эрхийн мөр нэмэх'}
              </CardTitle>
            ...
            <CardContent className="space-y-5">
                ...
                {selectedEmployee && (
                  <div className="mt-2 rounded-md border border-orange-200 bg-orange-50 px-3 py-2">
                    <p className="truncate text-sm font-medium text-orange-900">
                      {selectedEmployee.empname || `Ажилтан #${selectedEmployee.empid}`}
                    </p>
                    ...
                    {employeeOptions.map((employee) => (
                      ...
                      >
                        ...
                        <p className="truncate text-xs text-slate-500">
                          {employee.positionname || employee.email || employee.empid}
                        </p>
                      </button>
                    ))}
                  </div>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-800">Үйлдэл</label>
                <Select
                  value={draft.actionId}
                  onValueChange={(value) =>
                    setDraft((current) => ({ ...current, actionId: value }))
                  }
                >
              ...
              <div>
                ...
                <Select
                  value={draft.memberTypeId}
                  onValueChange={(value) =>
                    setDraft((current) => ({ ...current, memberTypeId: value }))
                  }
                >
                  <SelectTrigger className="w-full">
                  ...
                  <SelectContent>
                    {memberTypes.map((memberType) => (
                      <SelectItem
                        key={memberType.membertypeid}
                        value={String(memberType.membertypeid)}
                      >
                        {memberType.membertypename || `Төрөл #${memberType.membertypeid}`}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div className="flex gap-2">
                ...
                >
                  ...
                  {saving ? 'Хадгалж байна' : draft.id ? 'Өөрчлөлт хадгалах' : 'Тохиргоо нэмэх'}
                </Button>
                {draft.id && (
              ...
              {loading ? (
                ...
                <div className="divide-y divide-slate-100">
                  {filteredAssignments.map((assignment) => (
                    <div
                      key={assignment.id}
                      className={`grid min-w-0 gap-4 p-5 ${draft.id === assignment.id ? 'bg-orange-50/60' : 'hover:bg-slate-50'} md:grid-cols-[minmax(0,1fr)_180px_150px_80px] md:items-center`}
                    >
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-slate-900">
                          {assignment.empname || `Ажилтан #${assignment.empid}`}
                        </p>
                        <p className="mt-1 truncate text-xs text-slate-500">
                          {assignment.email || `ID: ${assignment.empid}`}
                        </p>
                      </div>
                      <div>
                        <p className="text-xs text-slate-500">Үйлдэл</p>
                        <Badge variant="secondary" className="mt-1 max-w-full truncate font-normal">
                          {assignment.actionname || assignment.actioncode}
                        </Badge>
                      </div>
                      <div>
                        <p className="text-xs text-slate-500">Үүрэг</p>
                        <p className="mt-1 truncate text-sm font-medium text-slate-800">
                          {assignment.membertypename || 'Тодорхойгүй'}
                        </p>
                      </div>

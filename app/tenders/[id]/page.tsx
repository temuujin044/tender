...
const requirementLabels = {
  required: 'Ерөнхий шаардлага',
  technical: 'Техникийн шаардлага',
  financial: 'Санхүүгийн шаардлага',
};
      ...
      try {
        ...
        setNotice(
          requestError instanceof Error
            ...
            : 'Нийлүүлэгчийн мэдээлэл ачаалж чадсангүй.'
        );
      }
    };
    void loadVendorData();
    if (new URLSearchParams(window.location.search).get('submitted'))
      setNotice(
        'Таны үнийн санал амжилттай хадгалагдлаа. Хяналтын самбараас явцыг хянах боломжтой.'
      );
  }, [tender]);
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
          <section>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-sm font-medium text-slate-500">{tender.invitationCode}</span>
              <span className="text-slate-400">•</span>
              <span className="text-sm text-slate-500">{tender.tenderCode ?? tender.id}</span>
            </div>
            <h1 className="mt-3 max-w-4xl text-3xl font-bold tracking-tight text-slate-900">
              {tender.title}
            </h1>
            ...
            <div className="mt-6 flex flex-wrap gap-x-6 gap-y-3 text-sm text-slate-600">
              ...
              <span className="flex items-center gap-2">
                ...
                Нийтэлсэн: {tender.publishDate}
              </span>
            </div>
          </section>

          <Card className="h-fit border-slate-200 shadow-sm lg:sticky lg:top-6">
            <CardContent className="p-6">
              ...
              <div className="mt-5 border-t border-slate-100 pt-5">
                <div className="flex items-center justify-between text-sm text-slate-500">
                  <span>Санал авах хугацаа</span>
                  <Clock3 className="h-5 w-5 text-orange-500" />
                </div>
                <p className="mt-1 font-semibold text-slate-900">{tender.deadline}</p>
                <p className="mt-1 text-xs text-slate-500">Нээх: {tender.openDate}</p>
              </div>
              {isOpen ? (
                  ...
                  {!participating ? (
                    <Link
                      href={
                        isAuthenticated
                          ? `/tenders/${id}/submit`
                          : getLoginRedirectPath(`/tenders/${id}/submit`)
                      }
                      className="block"
                    >
                    ...
                    <Link href={`/tenders/${id}/submit`} className="block">
                      <Button className="h-11 w-full bg-orange-500 hover:bg-orange-600">
                        <Send className="mr-2 h-4 w-4" />
                        {submissions.length ? 'Саналаа засах' : 'Үнийн санал илгээх'}
                      </Button>
                    </Link>
                  )}
                  <p className="text-center text-xs leading-5 text-slate-500">
                    {participating
                      ? 'Оролцох хүсэлт бүртгэгдсэн'
                      : 'Эхлээд оролцох хүсэлтээ баталгаажуулна'}
                  </p>
                </div>
              ) : (
                <div className="mt-6 rounded-lg bg-slate-100 p-3 text-center text-sm text-slate-600">
                  Санал хүлээн авах хугацаа дууссан
                </div>
              )}
            </CardContent>
          </Card>
        ...
        <Tabs defaultValue="overview" className="mt-10">
          <TabsList className="h-auto w-full justify-start gap-1 overflow-x-auto rounded-xl border border-slate-200 bg-white p-1">
            ...
            <TabsTrigger value="requirements">Шаардлага ({tender.requirements.length})</TabsTrigger>
            <TabsTrigger value="documents">Баримт ({tender.documents.length})</TabsTrigger>
            <TabsTrigger value="clarification">Тодруулга ({comments.length})</TabsTrigger>
            <TabsTrigger value="timeline">Үйл явц</TabsTrigger>
          </TabsList>

          <TabsContent value="overview" className="mt-5 grid gap-5 lg:grid-cols-2">
            <Card className="border-slate-200">
              <CardHeader>
                <CardTitle className="text-base">Урилгын мэдээлэл</CardTitle>
              </CardHeader>
              <CardContent>
                <dl className="space-y-4 text-sm">
                  {[
                    ['Урилгын код', tender.invitationCode],
                    ['Тендерийн код', tender.tenderCode ?? tender.id],
                    ['Тендерийн төрөл', tender.purchaseType],
                    ['Ангилал', tender.category],
                    ['Эхлэх огноо', tender.startDate],
                    ['Нээх огноо', tender.openDate],
                  ].map(([label, value]) => (
                    <div
                      key={label}
            ...
            <Card className="border-slate-200">
              <CardHeader>
                <CardTitle className="text-base">Таны оролцооны төлөв</CardTitle>
              </CardHeader>
              <CardContent>
                {submissions.length ? (
                  submissions.map((submission) => (
                    <div
                      key={submission.qouteid}
                      className="mb-3 rounded-xl border border-slate-200 p-4 last:mb-0"
                    >
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Хүргэлт: {submission.deliverydate || 'Товлоогүй'} •{' '}
                        {submission.deliveryday ?? 0} хоног
                      </p>
                    </div>
                  ))
                ) : (
                  <div className="rounded-xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">
                    Одоогоор үнийн санал илгээгээгүй байна.
                  </div>
                )}
              </CardContent>
            </Card>
                  ...
                  {tender.requirements.map((requirement) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          {requirementLabels[requirement.type]}
                          {requirement.documentRequired ? ' • Нотлох баримт хавсаргана' : ''}
                        </p>
                      </div>
                    </div>
                  ))}
          ...
          <TabsContent value="clarification" className="mt-5 grid gap-5 lg:grid-cols-[1fr_380px]">
            <Card className="border-slate-200">
              <CardHeader>
                <CardTitle className="text-base">Асуулт, хариулт</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {comments.length ? (
                  comments.map((comment) => (
                    <div
                      key={comment.id}
                      className={`rounded-xl border p-4 ${comment.author === 'buyer' ? 'border-orange-100 bg-orange-50/60' : 'border-slate-200 bg-white'}`}
                    >
                      <div className="flex items-center justify-between gap-3">
                        <p className="font-medium text-slate-900">{comment.title}</p>
                        <span className="text-xs text-slate-500">
                          {comment.author === 'buyer' ? 'Захиалагч' : 'Нийлүүлэгч'}
                        </span>
                      </div>
            ...
            <Card className="h-fit border-slate-200">
              <CardHeader>
                <CardTitle className="text-base">Тодруулга илгээх</CardTitle>
              </CardHeader>
              <CardContent>
                <form onSubmit={submitComment} className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="comment-title">Гарчиг</Label>
                    <Input
                      id="comment-title"
                      value={commentTitle}
                      onChange={(event) => setCommentTitle(event.target.value)}
                      placeholder="Асуултын товч гарчиг"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="comment-message">Асуулт</Label>
                    <Textarea
                      id="comment-message"
                      value={commentMessage}
                      onChange={(event) => setCommentMessage(event.target.value)}
                      placeholder="Тодруулах зүйлээ дэлгэрэнгүй бичнэ үү"
                      className="min-h-28"
                    />
                  </div>
                ...
                <div>
                  {tender.timeline.map((item, index) => (
                    ...
                    >
                      ...
                      <div
                        className={`relative z-10 mt-1 h-5 w-5 shrink-0 rounded-full border-4 ${item.complete ? 'border-orange-100 bg-orange-500' : 'border-slate-100 bg-slate-300'}`}
                      />
                      <div>

...
function invitationStatus(
  row: ApiInvitationRow,
  source: Exclude<CatalogScope, 'all'>
): TenderStatus {
...
export async function fetchTenderCatalog(
  ...
  const requests: { source: Exclude<CatalogScope, 'all'>; endpoint: string }[] =
    scope === 'all'
      ? [
          { source: 'saved', endpoint: 'getInvList' },
          { source: 'open', endpoint: 'getInvListOpen' },
          { source: 'result', endpoint: 'getInvListResult' },
        ]
      : [{ source: scope, endpoint: endpoints[scope] }];
  const groups = await Promise.all(
    requests.map(async ({ source, endpoint }) => ({
      source,
      rows: await fetchInvitationRows(endpoint),
    }))
  );
...
export async function fetchVendorTenders(vendorId: number): Promise<Tender[]> {
  const [participated, involved] = await Promise.all([
    apiRequest<ApiEnvelope<ApiInvitationRow[]>>(
      `/maktender/getInvListParticipated/?vendorid=${vendorId}`
    ),
    apiRequest<ApiEnvelope<ApiInvitationRow[]>>(
      `/maktender/getInvListInvolved/?vendorid=${vendorId}`
    ),
  ]);
  const byId = new Map<number, Tender>();
...
export async function fetchVendorNotifications(vendorId: number): Promise<VendorNotification[]> {
  ...
  return (unwrapRetData(response) ?? []).map((row) => {
    ...
    const type: VendorNotification['type'] =
      selected || status >= 7
        ? 'result'
        : participated
          ? 'status'
...
export async function fetchTenderDetail(invitationId: number): Promise<Tender> {
  ...
  const detailPromise = apiRequest<{ data: unknown[][] }>(
    `/maktender/invDetail/?invitationid=${invitationId}`
  );
  ...
  const fallback: Tender = base ?? {
    ...
    tenderId: Number(header.tenderid ?? 0),
    invitationId,
    invitationCode: clean(header.invitationcode, 'Урилгын код бүртгэгдээгүй'),
    tenderCode: clean(header.tendercode, 'Кодгүй'),
    title: clean(header.tendername, 'Нэр өгөөгүй тендер'),
    description: '',
    category: clean(header.tendertypename, 'Бусад'),
  ...
  return {
    ...
    timeline: [
      ...
      {
        event: clean(header.statusname, 'Одоогийн төлөв'),
        date:
          Number(header.balanceday ?? 0) >= 0
            ? `${header.balanceday} хоног үлдсэн`
            : 'Хугацаа дууссан',
        complete: Number(header.balanceday ?? -1) < 0,
      },
    ],
...
export async function loginAccount(username: string, password: string) {
  return apiRequest<AccountSession>('/tenderauth/login', {
    method: 'POST',
...
export async function fetchVendorProfile(vendorId: number) {
  const response = await apiRequest<ApiEnvelope<VendorProfileRecord>>(
    `/vendor/getInfo/${vendorId}/`
  );
...
export async function fetchComments(invitationId: number, vendorId: number) {
  const response = await apiRequest<{ comments?: ApiComment[] }>(
    `/comment/getComments/${invitationId}/${vendorId}/`
  );
...
export async function fetchQuotes(invitationId: number, vendorId: number) {
  const response = await apiRequest<{ quotes?: ApiQuote[] }>(
    `/quote/getList/${invitationId}/${vendorId}/`
  );
...
export async function joinTender(input: {
  invitationId: number;
  tenderId: number;
  vendorId: number;
  createdBy: string;
  ...
  const response = await apiRequest<{
    ...
    retType?: number;
    retMsg?: string;
  }>('/maktender/insertInvitationOfTender/', {
    method: 'POST',
    body: JSON.stringify({
  ...
  if (failed)
    throw new ApiError(response.RetMsg ?? response.retMsg ?? 'Оролцох хүсэлт хадгалагдсангүй.');
  return response;
}

export async function uploadTenderJoinDocument(input: {
  file: File;
  invitationId: number;
  tenderId: number;
  vendorId: number;
  ...
  return apiRequest<{
    data: { documentid: number; filename: string; sourceid: number; sourcetype: string };
  }>('/maktender/uploadTenderJoinDocument/', {
    method: 'POST',
    body: form,
  });
}
...
export async function fetchEmployeeTender(invitationId: number): Promise<EmployeeTender> {
  const detailRequest = apiRequest<{ data: unknown[][] }>(
    `/maktender/invDetail/?invitationid=${invitationId}`
  );
  const tender = await fetchTenderDetail(invitationId);
  const [initial, detailResponse] = await Promise.all([
    fetchTenderInitialData(tender.tenderId),
    detailRequest,
  ]);
  ...
  return {
    ...
    criteria: criteria
      ...
      .map((item) => {
        ...
        const type =
          Number(item.criteriatypeid) === 1
            ? 'financial'
            : Number(item.criteriatypeid) === 24
              ? 'experience'
              : 'technical';
        return { id: String(item.criteriaid), name, type, weight: Number(item.weight ?? 0) };
      }),
    documents: tender.documents.map((file, index) => ({
      id: String(file.sourceId ?? index),
      name: file.name,
      type: file.category,
      size: file.size,
    })),
...
export async function ensureEmployeeInvitation(
  tenderId: number,
  createdBy: string,
  invitationId = 0
) {
  const response = await apiRequest<ApiEnvelope<[ApiInvitationHeader[], unknown[]]>>(
    '/maktender/saveInvitationHeader/',
    {
      method: 'POST',
      body: JSON.stringify({
        tenderid: tenderId,
...
export async function uploadEmployeeTenderDocument(input: {
  file: File;
  tenderId: number;
  invitationId: number;
  createdBy: string;
  ...
  return apiRequest<{ data: { documentid: number; filename: string } }>(
    '/maktender/uploadTenderDocument/',
